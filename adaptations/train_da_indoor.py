...
from dataset.s3d8_dataset import Structured3D8DataSet
from dataset.stanford_pan8_dataset import StanfordPan8DataSetWeakStrong, StanfordPan8TestDataSet
from dataset.stanford_pin8_dataset import StanfordPin8DataSet
...
SAVE_PRED_EVERY = 1000  # for example 10000->250
SAVE_IMG_PRED_EVERY = 2500
...
IGNORE_LABEL = 255
INPUT_SIZE = '1024,512'
DATA_DIRECTORY_TARGET = '/home/jjiang/datasets/Stanford2D3D'
DATA_LIST_PATH_TARGET = 'dataset/s2d3d_pan_list/train.txt'
DATA_LIST_PATH_TARGET_TEST = 'dataset/s2d3d_pan_list/val.txt'
INPUT_SIZE_TARGET = '1024,512'
TARGET_TRANSFORM = 'resize'
INPUT_SIZE_TARGET_TEST = '2048,1024'
TARGET = 'span8'
SET = 'train'
NUM_CLASSES = 8
RANDOM_SEED = 1234
...
def main():
    ...
    RESTORE_FROM = '/home/jjiang/experiments/MultiMorph4PASS/final_ckpts_normed/indoor/source_models/2024-07-26-10-03_best_span8_model_65.97.pth'

    # change args
    PIN_DATA_DIRECTORY = '/home/jjiang/datasets/Stanford2D3D'
    PIN_DATA_LIST_PATH = 'dataset/s2d3d_pin_list/train.txt'
    Source_PIN_TRANSFORM = 'RandomCropWH'

    PAN_DATA_DIRECTORY = '/home/jjiang/datasets/Structured3D/Structured3D_2d_only'
    PAN_DATA_LIST_PATH = 'dataset/structured3d_list/train.txt'
    Source_PAN_TRANSFORM = 'resize'

    DIR_NAME = 'my_spin8s3d8_{}_{}_Morph_WarmUp_'.format(TARGET_NAME, MODEL)
    ...
    # init data loader
    trainset_pin = StanfordPin8DataSet(PIN_DATA_DIRECTORY, PIN_DATA_LIST_PATH,
    ...
    trainset_pan = Structured3D8DataSet(PAN_DATA_DIRECTORY, PAN_DATA_LIST_PATH,
                                        max_iters=args.num_steps * ORIGIN_BATCH_SIZE,
                                        crop_size=input_size, scale=args.random_scale, mirror=args.random_mirror,
                                        mean=IMG_MEAN, set=args.set, need_grey=base_channel == 1,
                                        trans=Source_PAN_TRANSFORM)
    ...
    targetset = StanfordPan8DataSetWeakStrong(args.data_dir_target, args.data_list_target,
                                              max_iters=args.num_steps * ORIGIN_BATCH_SIZE,
                                              crop_size=input_size_target, scale=False, mirror=args.random_mirror,
                                              mean=IMG_MEAN,
                                              set=args.set,
                                              trans=TARGET_TRANSFORM, need_grey=base_channel == 1,
    ...
    targettestset = StanfordPan8TestDataSet(args.data_dir_target, args.data_list_target_test,
    ...
    for i_iter in range(Iter, args.num_steps + 1):
        ...
        images_target, images_target_strong, images_target_grey, _, _ = batch_target
        ...
        images_target_cropped = images_target
        ...
        with torch.no_grad():
            ...
            labels_target_cropped_ensemble = labels_target_ensemble
        ...
        loss_adv_target = bce_loss(D_out,
                          ...
                          bce_loss(D_out_full_pin2pan,
                                   torch.FloatTensor(D_out_full_pin2pan.data.size()).fill_(
