...
class RandomHorizontalFlip(object):
    def __init__(self):
        self.do_trans = random.random() < 0.5

    def __call__(self, sample, is_label=False):
        if self.do_trans:
...
class RandomRotate(object):
    def __init__(self, degree):
        self.degree = degree
        self.rotate_degree = random.uniform(-1 * self.degree, self.degree)

    def __call__(self, sample, is_label=False):
        inter_type = Image.NEAREST if is_label else Image.BILINEAR
        sample = sample.rotate(self.rotate_degree, inter_type)
        return sample
...
class FixScaleCropWH_Center(object):
    ...
    def __call__(self, sample, is_label=False):
        ...
        if w < cw:
            ...
            nw = cw
            nh = int(1.0 * h * nw / w)
            sample = sample.resize((nw, nh), Image.NEAREST if is_label else Image.BILINEAR)
        w, h = sample.size
        if h < ch:
            # new w h
            nh = ch
...
class FixScaleRandomCropWHBoth(object):
    def __init__(self, sample_size_wh, crop_size_wh):
        assert isinstance(crop_size_wh, tuple)
        self.sample_size_wh = sample_size_wh
        self.crop_size = crop_size_wh
        w, h = self.sample_size_wh
        cw, ch = self.crop_size
        if w < cw:
            # new w h
            nw = cw
        ...
        # random crop crop_size
        self.x1 = random.randint(0, self.nw - self.crop_size[0])
        self.y1 = random.randint(0, self.nh - self.crop_size[1])

    def __call__(self, sample, is_label=False):
        sample = sample.resize((self.nw, self.nh), Image.NEAREST if is_label else Image.BILINEAR)
...
class TensorFixScaleRandomCropWHBoth(object):
    def __init__(self, sample_size_wh, crop_size_wh):
        assert isinstance(crop_size_wh, tuple)
        self.sample_size_wh = sample_size_wh
        self.crop_size = crop_size_wh
    ...
    def __call__(self, sample, is_label=False):
        if is_label:
            sample = sample[:, None, :, :]
            sample = torch.nn.functional.interpolate(sample.float(), size=(self.nh, self.nw), mode='nearest')
        else:
            sample = torch.nn.functional.interpolate(sample, size=(self.nh, self.nw), mode='bilinear')

        # left, upper, right, and lower
        sample = sample[:, :, self.y1:self.y1 + self.crop_size[1], self.x1:self.x1 + self.crop_size[0]]
        return sample.squeeze(1).long() if is_label else sample
