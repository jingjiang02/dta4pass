...
import adaptations.morph.TransMorph as TM
import adaptations.morph.configs_TransMorph_diff_rgb as configs
import adaptations.morph.finite_differences as fdt
...
class TransMorphDiffRGB(nn.Module):
    ...
    def __init__(self, config, channel=3, use_grad=False, input_size=None, window_size=None):
        super(TransMorphDiffRGB, self).__init__()
        self.channel = channel
        self.origin_channel = self.channel
        self.use_grad = use_grad
        if self.use_grad:
            self.channel = self.channel + 2
        config.in_chans = config.in_chans * self.channel
        img_sz = config.img_size if input_size is None else input_size
        window_size = config.window_size if window_size is None else window_size
    ...
    def forward(self, inputs):
        ...
        source_image = source[:, :self.origin_channel, :, :]
        target_image = target[:, :self.origin_channel, :, :]
        ...
        inverse_flow = inverse_flow * self.scale
        for _ in range(self.int_steps):
    ...
    def __do_some_clean(self):
        ...
        self.inverse_res_flow_mean = None
    ...
    def prec_loss_with_weight(self, disp, weight):  ##!!!!!!!!
        """
        a more manual implementation of the precision matrix term
                mu * P * mu    where    P = D - A
        where D is the degree matrix and A is the adjacency matrix
        ...
        l2 = l2.to(weight.device) * weight
        # l2.shape=torch.Size([1, 256, 512])
        reg = l2.mean()
        return reg * 0.5

    def kl_loss_with_weight(self, weight):
        """
        KL loss
        y_pred is assumed to be D*2 channels: first D for mean, next D for logsigma
        D (number of dimensions) should be 1, 2 or 3
        y_true is only used to get the shape
        """
        # weight.shape=[B, H, W]=torch.Size([1, 256, 512])
        # prepare inputs
        ndims = 3
        flow_mean = self.res_flow_mean
        inverse_flow_mean = self.inverse_res_flow_mean
        ...
        # sigma terms
        sigma_term = self.prior_lambda * self.D * torch.exp(log_sigma) - log_sigma  ##!!!!!!!!
        # sigma_term.shape=torch.Size([1, 2, 256, 512])
        sigma_term = sigma_term * weight[:, None, :, :].repeat(1, 2, 1, 1)
        sigma_term = torch.mean(sigma_term)  ##!!!!!!!!

        # precision terms
        prec_term = self.prior_lambda_mean * (
                self.prec_loss_with_weight(flow_mean, weight) + self.prec_loss_with_weight(inverse_flow_mean,
                                                                                           weight))  # this is the jacobi loss
