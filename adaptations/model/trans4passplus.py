...
class Trans4PASSOneShareBackboneMoE(nn.Module):
    def __init__(self, num_classes=19, emb_chans=128, encoder='trans4pass_v2', num_source_domains=2, norm=None):
        super().__init__()
        if encoder == 'trans4pass_v1':
            self.encoder = trans4pass_v1()
        elif encoder == 'trans4pass_v2':
        ...
        self.dede_heads = torch.nn.ModuleList([DMLP(num_classes, emb_chans) for _ in range(self.num_source_domains)])

        hidden_dim = 128
        self.moe_conv = torch.nn.ModuleList([
            torch.nn.Conv2d(emb_chans * num_source_domains, hidden_dim, 3, 1, 1),
            torch.nn.BatchNorm2d(hidden_dim),
            torch.nn.ReLU(),
            torch.nn.Conv2d(hidden_dim, num_source_domains, 3, 1, 1),
        ])

        self.pred = torch.nn.Conv2d(emb_chans, num_classes, 1)
    ...
    def forward(self, x, ensemble=False):
        ...
        if ensemble:
            x_output = x_all + [x]
            return sum(x_output) / len(x_output)
        else:
            return x_all, x
