...
class StyleGAN2Discriminator(nn.Module):
    def __init__(self, input_nc, size, ndf=64):
        super().__init__()
        ...
        self.final_linear = nn.Sequential(
            EqualLinear(channels[4] * 8, channels[4], activation='fused_lrelu'),
...
class StyleGAN2DiscriminatorUNet(nn.Module):
    def __init__(self, input_nc, size, ndf=64, pixel_factor=1):
        super().__init__()
        ...
        convs = [ConvLayer(input_nc, channels[size], 1)]
        unet_convs = [ConvLayer(channels[size] * 2, 1, 1, activate=False)]
        ...
        for i in range(log_size, final_res_log2, -1):
            ...
            if final_res_log2 + 1 < i <= log_size:
                unet_convs.append(ResBlock(out_channel * 2, in_channel, blur_kernel, downsample=False))
            elif i == final_res_log2 + 1:
                unet_convs.append(ResBlock(out_channel, in_channel, blur_kernel, downsample=False))

            in_channel = out_channel

        self.convs = nn.Sequential(*convs)
        unet_convs = unet_convs[::-1]

        pow_pixel_factor = int(math.log(1 / pixel_factor, 2))
        unet_convs = unet_convs[:-(pow_pixel_factor + 1)]
        unet_convs.append(ConvLayer(int(32 / pixel_factor), 1, 1, activate=False))
        self.unet_convs = nn.Sequential(*unet_convs)

        self.final_conv = nn.Sequential(
            ResBlock(384, 384, blur_kernel),
            ResBlock(384, 384, blur_kernel),
            ResBlock(384, 384, blur_kernel),
    ...
    def forward(self, input):
        ...
        B, C, H, W = out.shape
        out = out.reshape([B, C * H * W])
        middle_out = self.final_linear(out)

        # final pixel out
        saved_out = saved_out[::-1]
        out = saved_out[0]
        out = self.unet_convs[0](out)
        for i, conv in enumerate(self.unet_convs[1:]):
            out = F.interpolate(out, scale_factor=2, mode="bilinear")
            out = torch.cat([out, saved_out[i + 1]], dim=1)
            out = conv(out)

        return out, middle_out


class StyleGAN2DiscriminatorForFisheye(nn.Module):
    def __init__(self, input_nc, size, ndf=64):
        super().__init__()
        self.stddev_group = 16
        ...
        channel_multiplier = ndf / 64
        channels = {
            4: min(256, int(4096 * channel_multiplier)),
            8: min(256, int(2048 * channel_multiplier)),
            16: min(256, int(1024 * channel_multiplier)),
            32: min(256, int(512 * channel_multiplier)),
            64: int(256 * channel_multiplier),
            128: int(128 * channel_multiplier),
            256: int(64 * channel_multiplier),
            512: int(32 * channel_multiplier),
        ...
        self.final_linear = nn.Sequential(
            EqualLinear(int(channels[4] * 9), channels[4], activation='fused_lrelu'),
            EqualLinear(channels[4], 1),
        )
