...
class SegmentationDataset(object):
    ...
    def _sync_transform_all(self, img, image_grey, mask=None):
        ...
        elif self.trans == 'resize_crop':
            crop = ResizeCrop(img.size, self.crop_size)
            img = crop(img)
            image_grey = crop(image_grey)
            if mask is not None:
                mask = crop(mask, is_label=True)
        else:
            raise Exception
        ...
        if random.random() < 0.5:
            img = img.transpose(Image.FLIP_LEFT_RIGHT)
            image_grey = image_grey.transpose(Image.FLIP_LEFT_RIGHT)
            if mask is not None:
                mask = mask.transpose(Image.FLIP_LEFT_RIGHT)

        if mask is not None:
            return img, image_grey, mask
        else:
            return img, image_grey
